...
(function () {
  ...
  function drawTextAroundArc(ctx, text, centerX, centerY, radius, angleOffset) {
    ctx.save();
    ctx.translate(centerX, centerY);  // Chuyển tới tâm của vòng tròn
    ctx.rotate(angleOffset);  // Xoay canvas để bắt đầu vẽ từ góc mong muốn

    let angle = (Math.PI * 2) / text.length;  // Tính toán góc cho mỗi ký tự

    for (let i = 0; i < text.length; i++) {
        ctx.save();
        ctx.rotate(i * angle);  // Xoay theo từng ký tự
        ctx.translate(0, -radius);  // Đẩy ký tự ra khỏi tâm tới đúng bán kính
        ctx.fillText(text[i], -ctx.measureText(text[i]).width / 2, 0);  // Vẽ ký tự tại vị trí hiện tại
        ctx.restore();
    }
